  ...
  <section id="chat">
    <aside class="bg-base-200">
      <router-link :to="`/setting/${user_chats.userId}`">
        <main>
          ...
          />
          <div>
            <p class="text-xs">{{ user_chats.userName }}</p>
            <p
              class="text-xs badge text-white"
              :class="user_chats.seen === 'online' ? 'badge-success' : 'badge-error'"
            >
              {{ user_chats.seen }}
            </p>
          </div>
    ...
    <aside>
      ...
      >
        ...
        <div class="chat-bubble text-xs">{{ chats.start ? chats.start : chats.end }}</div>
